...
% In this case, the unit cell consists of four square 'air-hole' unit cells
% tiled in a 2x2 grid.
...
% CHANGE UNIT CELL AND VALUES TO HAVE QUAD UNIT CELL!
eGrid = [eGrid(:,:),eGrid(:,:);eGrid(:,:),eGrid(:,:)];
uGrid = [uGrid(:,:),uGrid(:,:);uGrid(:,:),uGrid(:,:)];
...
points = [{[0;0],"\Gamma"}; {ahash_,"X"}; {(ahash_+bhash_),"M"}];
...
central_index_P = (P+1)/2;
physical_indices_P = central_index_P + 2*(-floor(P/4):floor(P/4));
central_index_Q = (Q+1)/2;
physical_indices_Q = central_index_Q + 2*(-floor(Q/4):floor(Q/4));

NSH_physical = (1+floor(P/4)*2)*(1+floor(Q/4)*2);    % Number of subharmonics - periodic on double cell, but not primitive cell
NSH_nonphysical = NH-NSH_physical;

SHInds_phys = reshape(physical_indices_P'+(physical_indices_Q-1)*P,[1,NSH_physical]);
...
maxrange = max([-pi/Lx, pi/Lx, -pi/Ly, pi/Ly]*2);
...
for t = (1:NSH_physical)
    ...
    %p = patchline(dists(2:end),DH_dat_sorted_p(:,t),'edgecolor','r','linestyle','-','edgealpha',0.5); % plot H/TE modes (red)
    p.LineWidth = 2;
    %p.Color(4) = 0.5;
    p = plot(dists(2:end),DE_dat_sorted_p(:,t),'b-'); % plot E/TM modes (blue)
    %p = patchline(dists(2:end),DE_dat_sorted_p(:,t),'edgecolor','b','linestyle','-','edgealpha',0.5); % plot E/TM modes (blue)
    p.LineWidth = 2;
    %p.Color(4) = 0.5;
end
